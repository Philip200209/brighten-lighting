...
const ADMIN_EMAIL = vars.VITE_ADMIN_EMAIL;
const ADMIN_PASSWORD = vars.VITE_ADMIN_PASSWORD;
...
(async () => {
  try {
    // If admin credentials are provided in .env.local, sign in so RLS checks
    // that rely on auth.uid() will see the admin profile.
    if (ADMIN_EMAIL && ADMIN_PASSWORD) {
      console.log('Signing in as admin for RLS-checked operations...');
      const { data: signData, error: signErr } = await supabase.auth.signInWithPassword({
        email: ADMIN_EMAIL,
        password: ADMIN_PASSWORD,
      });
      if (signErr) {
        console.error('Admin sign-in failed:', signErr.message || signErr);
      } else {
        console.log('Admin sign-in succeeded');
      }
    } else {
      console.log('No admin credentials provided; running as anon key');
    }
